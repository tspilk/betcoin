...
uint256 hashGenesisBlock("0x806108e1d3f5604a08a83b081af7ddc2529dfd0f38b5d38f436e422dad23d5d6");
...
{
    // BETCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // BETCoin: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // BETCoin: 3.5 days
static const int64 nTargetSpacing = 5.0 * 60; // BETCoin: 5.0 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BETCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BETCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BETCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x806108e1d3f5604a08a83b081af7ddc2529dfd0f38b5d38f436e422dad23d5d6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This will be used for betting at bars. 2013-12-14 creation date";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        // Find a Genesis block if it doesnt match
        if (false && block.GetHash() != hashGenesisBlock) // set it to false so no genesis is hashed (even though it wont)
        {
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // BETCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BETCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BETCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BETCoinMiner(CWallet *pwallet)
{
    printf("BETCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running BETCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BETCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BETCoinMiner, pwallet));
}
